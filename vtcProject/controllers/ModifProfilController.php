...
if(isset($_POST['submit'])){
    ...
    empty($_POST['sexe'])|| empty($_POST['wilaya'])||empty($_POST['password'])||empty($_POST['confirm'])|| (!empty($_POST['oui']) && (empty($_POST['wilayaArriver']))) ){
        if(!empty($_POST['oui']) && (empty($_POST['wilayaArriver']))){
            header('Location: ../views/ModifProfil.php?message=5');   
           
        }
       header('Location: ../views/ModifProfil.php?message=1');
      
    }
    else{
        if (check_mdp_format($_POST['password']) != true)
        {
            header('Location: ../views/ModifProfil.php?message=2');	
        }
        else{
                ...
                if(is_uploaded_file($_FILES['image']['tmp_name'])){
                    $image_tmp = $_FILES["image"]["tmp_name"];
                     ...
                     $image = addslashes(file_get_contents($_FILES['image']['tmp_name']));
                         ...
                         $model->updateInfoUser($nom,$prenom,$datenaissance,$sexe,$adresse,$wilaya,$tel,$mail,$password,$type,$image,$_SESSION['matricule']);
                            ...
                            if($type == 'Transporteur'){
                                ...
                                $rslt=$mode-> getWilayaUser($_SESSION['matricule']);
                                if($rslt != 'vide'){
                                    $mode->deleteWilayaUser($_SESSION['matricule']);
                                }
                    ...
                    header('Location: ../views/ModifProfil.php?message=3');
                  }
        }
    }
}
